...
이 코드는 상품, 고객, 주문 클래스를 정의하고, 이들 클래스를 사용하여 상품 주문 관리 시스템을 구현한 예제입니다.

이 시스템은 사용자에게 상품명, 가격, 고객명, 주소, 주문 수량을 입력받고, 이 정보를 사용하여 상품, 고객, 주문 객체를 생성합니다. 이후, 사용자는 상품을 추가하거나 삭제할 수 있습니다. 추가할 때는 상품명, 가격, 추가 수량을 입력받고, 삭제할 때는 상품명 또는 인덱스를 입력받습니다. 마지막으로, 현재 주문 내역과 상품 리스트를 출력합니다.

구성 요소:

Person 클래스: 이름과 주소 정보를 가지는 클래스입니다. 상속 클래스를 위한 기본 클래스 역할을 합니다.
Product 클래스: 상품 정보를 가지는 클래스입니다. Person 클래스를 상속받습니다.
Customer 클래스: 고객 정보를 가지는 클래스입니다. Person 클래스를 상속받습니다.
Order 클래스: 주문 정보를 가지는 클래스입니다. Customer와 Product 객체를 이용하여 생성됩니다.
동작:

사용자로부터 상품명, 가격, 고객명, 주소, 주문 수량을 입력받습니다.
상품, 고객, 주문 객체를 생성합니다.
사용자로부터 추가할 상품이 있는 경우, 상품명, 가격, 추가 수량을 입력받고, 상품을 추가합니다.
사용자로부터 삭제할 상품이 있는 경우, 상품명 또는 인덱스를 입력받고, 해당 상품을 삭제합니다.
현재 주문 내역과 상품 리스트를 출력합니다.
