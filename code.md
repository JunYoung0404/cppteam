...
이 코드는 상품, 고객, 주문 클래스를 정의하고 이를 이용해 객체를 생성하고 관리하는 예제 코드입니다. main 함수에서는 사용자로부터 상품, 고객, 주문에 대한 정보를 입력받아 이를 기반으로 Product, Customer, Order 객체를 생성하고 출력합니다.

또한, 상품을 추가하거나 삭제하고 고객 정보를 변경하는 기능을 구현한 부분이 있습니다. 이는 while문을 통해 메뉴를 계속해서 출력하며 사용자로부터 입력을 받아 기능을 수행합니다.

주요 클래스 및 함수 설명:

Person: 이름(name)과 주소(address) 정보를 가지는 클래스
Product: 이름(name)과 가격(price) 정보를 가지며 Person 클래스를 상속받음
Customer: 이름(name)과 주소(address) 정보를 가지며 Person 클래스를 상속받음
Order: 고객(Customer)과 상품(Product) 정보를 가지며 주문 수량(quantity)도 함께 저장됨. 상품 추가, 삭제, 고객 정보 변경 기능이 포함됨
main: 사용자로부터 입력을 받아 Product, Customer, Order 객체를 생성하고 출력하는 함수. 또한, while문을 이용해 상품 추가, 삭제, 고객 정보 변경 기능을 구현함.
